...
※장점
검색 속도 향상 (WHERE, JOIN 조건 시 빠른 접근)
정렬 속도 향상
유일성 제약 시 중복 검사 가능
※단점
인덱스 생성/유지에 추가 저장 공간 필요
INSERT, UPDATE, DELETE 시 인덱스도 갱신되어 오버헤드 발생
너무 많은 인덱스는 오히려 성능 저하
**18 인덱스를 사용하는 것이 바람직한 경우와 그렇지 않은 경우를 구분하여 설명하시오.**
인덱스는 데이터 검색 속도를 향상시키기 위해 사용되며, 조건 검색, 정렬, 조인 등이 자주 발생하는 컬럼에 설정하는 것이 바람직하다. 
그러나 데이터의 삽입, 수정, 삭제가 잦거나 데이터량이 적은 테이블에는 오히려 성능을 저하시킬 수 있다. 
또한, 와일드카드가 앞에 오는 검색이나 과도한 인덱스 사용은 비효율적이다. 
따라서 인덱스는 상황에 따라 신중하게 사용하는 것이 중요하다.
